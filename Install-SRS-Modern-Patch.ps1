...
function Test-PatchPackageRoot {
    param([string]$PackageRoot)

    if ([string]::IsNullOrWhiteSpace($PackageRoot)) { return $false }
    if (!(Test-Path -LiteralPath (Join-Path $PackageRoot "patch\dinput8.dll"))) { return $false }
    $CandidateAssetDir = Join-Path $PackageRoot "srs-modern-patch-assets"
    foreach ($AssetName in $AssetNames) {
        if (!(Test-Path -LiteralPath (Join-Path $CandidateAssetDir $AssetName))) {
            return $false
        }
    }
    return $true
...
function Resolve-PatchPackageRoot {
    $candidates = @(
        $Root,
        (Join-Path $Root "package\SRS-Modern-Patch")
    )

    foreach ($candidate in $candidates) {
        if (Test-PatchPackageRoot $candidate) {
            return $candidate
        }
    }

    $releaseZip = Join-Path $Root "release\SRS-Modern-Patch.zip"
    if (Test-Path -LiteralPath $releaseZip) {
        $tempRoot = Join-Path $env:TEMP ("SRS-Modern-Patch-" + [Guid]::NewGuid().ToString("N"))
        New-Item -ItemType Directory -Force -Path $tempRoot | Out-Null
        Expand-Archive -LiteralPath $releaseZip -DestinationPath $tempRoot -Force
        $extractedRoot = Join-Path $tempRoot "SRS-Modern-Patch"
        if (Test-PatchPackageRoot $extractedRoot) {
            return $extractedRoot
        }
    }

    throw "Missing patch files. Expected patch\dinput8.dll and srs-modern-patch-assets next to the installer, in package\SRS-Modern-Patch, or inside release\SRS-Modern-Patch.zip."
}
...
$PatchPackageRoot = Resolve-PatchPackageRoot
$PatchDll = Join-Path $PatchPackageRoot "patch\dinput8.dll"
$AssetDir = Join-Path $PatchPackageRoot "srs-modern-patch-assets"
