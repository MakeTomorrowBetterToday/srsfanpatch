...
public:
    ...
    HRESULT STDMETHODCALLTYPE CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface) override
    {
        ...
        if (pPresentationParameters && hwnd) {
            ...
            UINT targetW = (UINT)(mr.right - mr.left);
            UINT targetH = (UINT)(mr.bottom - mr.top);
            if (targetW < 640) targetW = pPresentationParameters->BackBufferWidth;
            if (targetH < 480) targetH = pPresentationParameters->BackBufferHeight;
            g_srsDrawWidth = pPresentationParameters->BackBufferWidth;
            g_srsDrawHeight = pPresentationParameters->BackBufferHeight;

            FitWindowToNearestMonitor(hwnd);
            pPresentationParameters->hDeviceWindow = hwnd;
            pPresentationParameters->Windowed = TRUE;
            if (g_forcedBackBufferWidth && g_forcedBackBufferHeight) {
                pPresentationParameters->BackBufferWidth = g_forcedBackBufferWidth;
                pPresentationParameters->BackBufferHeight = g_forcedBackBufferHeight;
                char forceBuf[128];
                wsprintfA(forceBuf, "forced SRS backbuffer to %ux%u", g_forcedBackBufferWidth, g_forcedBackBufferHeight);
                LogLine(forceBuf);
            }
...
static HRESULT STDMETHODCALLTYPE SrsDeviceSetViewport(IDirect3DDevice9* device, const D3DVIEWPORT9* viewport)
{
    ...
    if (g_fitRect.right > g_fitRect.left && g_fitRect.bottom > g_fitRect.top) {
    ...
    if (now - g_lastViewportLogTick > 3000) {
        ...
        char buf[224];
        if (viewport) {
            wsprintfA(buf, "SetViewport forced from X=%lu Y=%lu W=%lu H=%lu to W=%lu H=%lu",
                viewport->X, viewport->Y, viewport->Width, viewport->Height, forced.Width, forced.Height);
        } else {
            wsprintfA(buf, "SetViewport got null, forcing W=%lu H=%lu", forced.Width, forced.Height);
        }
        LogLine(buf);
    }
...
static void PatchDeviceVtable(IDirect3DDevice9* device)
{
    ...
    DWORD oldProtect = 0;
    if (!VirtualProtect(&vtbl[17], sizeof(void*) * 31, PAGE_READWRITE, &oldProtect)) {
    ...
    DWORD ignored = 0;
    VirtualProtect(&vtbl[17], sizeof(void*) * 31, oldProtect, &ignored);
    FlushInstructionCache(GetCurrentProcess(), &vtbl[17], sizeof(void*) * 31);
    InterlockedExchange(&g_deviceVtablePatched, 1);
    LogLine("patched IDirect3DDevice9 Present and SetViewport vtable entries");
}
...
static void PatchDevicePresentVtable(IDirect3DDevice9* device)
{
    ...
    DWORD oldProtect = 0;
    if (!VirtualProtect(&vtbl[17], sizeof(void*), PAGE_READWRITE, &oldProtect)) {
        LogLine("VirtualProtect failed for IDirect3DDevice9 Present vtable");
        return;
    }
    ...
    DWORD ignored = 0;
    VirtualProtect(&vtbl[17], sizeof(void*), oldProtect, &ignored);
    FlushInstructionCache(GetCurrentProcess(), &vtbl[17], sizeof(void*));
    InterlockedExchange(&g_deviceVtablePatched, 1);
    LogLine("patched IDirect3DDevice9 Present vtable entry");
}
